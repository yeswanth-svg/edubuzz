...
<div class="container my-5">
    <div class="row">
        ...
        <div class="col-md-8 col-lg-9">
            <div class="text-center mb-4" id="subject-buttons">
                @foreach($subjects as $subject)
                    <button class="subject-button {{ $loop->first ? 'active' : '' }} {{ strtolower($subject->name) }}"
                        data-subject="{{ strtolower($subject->name) }}">
                        {{ $subject->name }}
                    </button>
                @endforeach
            </div>
            <div id="subjects-container" class="row">
                @foreach($subjects as $subject)
                    <div class="subject-box {{ $loop->first ? 'show' : '' }}"
                        data-subject="{{ strtolower($subject->name) }}">
                        <div class="row">
...
<script>
    $(document).ready(function () {
        ...
        // Handle button clicks for subjects
        $('.subject-button').on('click', function () {
            ...
            // Hide all subject boxes and show only the selected subject
            $('.subject-box').removeClass('show');
            $('.subject-box[data-subject="' + selectedSubject + '"]').addClass('show');
        });
    });
</script>
